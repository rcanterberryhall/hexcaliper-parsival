...
def _make_win32_stack(inbox_messages: list, sent_messages: list = None):
    ...
    mock_ns = MagicMock()
    mock_ns.GetDefaultFolder.side_effect = lambda fid: inbox_folder if fid == 6 else sent_folder
...
def _always_500() -> MagicMock:
    ...
    mock_resp.raise_for_status.side_effect = req.HTTPError("500 Server Error", response=mock_resp)
    return mock_resp
...
def test_seed_and_infer_reaches_review_and_prints_ui_url(capsys):
    ...
    with patch.dict(
        ...
        sidecar = _sidecar()
        with (
            patch("outlook_sidecar.requests.post", side_effect=[ingest_resp, seed_start_resp]),
...
def test_seed_and_infer_exits_when_api_unreachable():
    ...
    with patch.dict(
        ...
        with (
            patch("outlook_sidecar.requests.post", side_effect=[ingest_resp, req.ConnectionError]),
            pytest.raises(SystemExit),
        ):
            sidecar._seed_and_infer()
