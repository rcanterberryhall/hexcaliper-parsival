...
def _seed_corpus():
    ...
    records = [
        _analysis(
            "r1",
            "SHIFT HIGHLIGHTS — Reactor A coolant pressure drop",
        ...
        _analysis(
            "r2",
            "Re: Reactor A — PROJ-88 valve inspection",
            "Valve showing wear. Parts ordered.",
            priority="high",
            body_preview="reactor A valve PROJ-88",
        ),
        _analysis(
            "r3",
            "SHIFT ACTIVITIES — Reactor A stable",
            "Back online. All parameters nominal.",
            is_passdown=True,
            body_preview="reactor A online nominal",
        ),
        _analysis(
            "r4",
            "PROJ-88 parts arrived — scheduling replacement",
            "Spares in stores. Scheduling Saturday window.",
            body_preview="reactor PROJ-88 valve replacement Saturday",
        ),
        _analysis(
            "r5",
            "Reactor A coolant trending down again",
            "Pressure at 93%. May need to accelerate PROJ-88.",
            priority="high",
            body_preview="reactor A coolant PROJ-88",
        ),
        _analysis(
            "s1",
            "Safety audit Zone 3 — findings report",
            "Six findings. Two high-priority items require CAP.",
            priority="high",
            body_preview="safety audit zone 3 corrective action CAP",
        ),
        _analysis(
            "s2",
            "Re: Safety audit — CAP draft submitted",
            "Draft corrective action plan under review.",
            body_preview="safety audit corrective action plan",
        ),
        _analysis(
            "s3",
            "SHIFT HIGHLIGHTS — Safety walkthrough complete",
            "Daily safety walkthrough done. No new findings.",
            is_passdown=True,
            body_preview="safety walkthrough zone 3",
        ),
        _analysis(
            "s4",
            "Safety audit CAP — sign-off required",
            "CAP needs signature before regulator deadline.",
            priority="high",
            body_preview="safety audit CAP sign-off",
        ),
        _analysis(
            "n1",
            "Friday lunch menu",
            "Cafeteria special: lasagne.",
            priority="low",
            category="noise",
    ...
    map_response = _ollama_mock(
        {
            ...
            "concerns": ["safety audit CAP"],
        }
    )
    reduce_response = _ollama_mock(
        {
            "projects": [{"name": "Reactor Upgrade", "keywords": ["reactor", "PROJ-88"]}],
            "topics": ["safety audit CAP"],
        }
    )
