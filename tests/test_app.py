...
def test_patch_todo_edits_description_and_deadline(client):
    ...
    r = client.patch(
        f"/todos/{doc_id}", json={"description": "Updated desc", "deadline": "2026-06-01"}
    )
    assert r.status_code == 200
...
def test_scan_starts_and_returns_sources(client):
    ...
    with (
        patch("orchestrator.connector_slack.fetch", return_value=[]),
...
def _make_analysis(
    item_id="z1", has_action=False, category="fyi", action_items=None, information_items=None
):
    return Analysis(
        item_id=item_id,
        source="outlook",
...
def test_save_analysis_does_not_duplicate_todos():
    ...
    a = _make_analysis("z2", has_action=True, category="task", action_items=[action])
    _save_analysis(a)
    _save_analysis(a)
...
def test_save_analysis_does_not_duplicate_intel():
    ...
    a = _make_analysis(
        "z3", information_items=[{"fact": "Server was rebooted", "relevance": "ops context"}]
    )
    _save_analysis(a)
    _save_analysis(a)
...
def test_patch_analysis_priority_reason_records_override(client):
    ...
    _insert_analysis(item_id="po1", priority="low", author="boss@example.com", title="Weekly sync")
    r = client.patch(
        "/analyses/po1", json={"priority": "high", "priority_reason": "person_matters"}
    )
    assert r.status_code == 200
    ...
    assert config.PRIORITY_OVERRIDES and config.PRIORITY_OVERRIDES[-1]["reason"] == "person_matters"
...
def test_patch_analysis_invalid_priority_reason_ignored(client):
    ...
    r = client.patch("/analyses/po2", json={"priority": "high", "priority_reason": "banana"})
    assert r.status_code == 200
    saved = _db.get_settings()
...
def test_patch_analysis_priority_reason_without_priority_change_ignored(client):
    ...
    r = client.patch("/analyses/po3", json={"priority": "high", "priority_reason": "deadline_real"})
    assert r.status_code == 200
    saved = _db.get_settings()
    assert not saved.get("priority_overrides")
...
def test_get_briefing_returns_cached_content(client):
    ...
    content = {
        "sections": [{"project": "Alpha", "summary": "All good.", "situations": [], "todos": []}]
    }
    with db.lock:
...
class TestManualTodoMigration:
    ...
    def test_backfill_creates_items_row_for_orphan_manual_todo(self, client):
        ...
        with _db.lock:
            row = _db.conn().execute("SELECT item_id FROM todos WHERE id = ?", (tid,)).fetchone()
    ...
    def test_backfill_skips_non_manual_todos(self, client):
        ...
        with _db.lock:
            ...
            count = (
                _db.conn()
                .execute("SELECT COUNT(*) FROM items WHERE item_id LIKE 'manual_%'")
                .fetchone()[0]
            )
        assert count == 0
...
class TestManualTodoCreationSynthesizesItem:
    ...
    def test_post_todos_creates_items_row(self, client):
        ...
        with _db.lock:
            todo_row = (
                _db.conn().execute("SELECT item_id FROM todos WHERE id = ?", (doc_id,)).fetchone()
            )
...
class TestPatchAnalysisRichFields:
    ...
    def test_patch_accepts_body_preview(self, client):
        self._seed()
        resp = client.patch("/analyses/edit_me", json={"body_preview": "free-form notes here"})
        assert resp.status_code == 200
        import db as _db
    ...
    def test_patch_accepts_goals_list(self, client):
        self._seed()
        resp = client.patch(
            "/analyses/edit_me", json={"goals": ["draft proposal", "review metrics"]}
        )
        assert resp.status_code == 200
        import json as _json
