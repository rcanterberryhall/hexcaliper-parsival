...
def test_parse_header_pairs_rfc_style():
    pairs = contacts.parse_header_pairs('"Jane Doe" <jane@acme.com>, Bob <bob@acme.com>')
    assert pairs == [("Jane Doe", "jane@acme.com"), ("Bob", "bob@acme.com")]


def test_parse_header_pairs_falls_back_to_bare_addresses():
    pairs = contacts.parse_header_pairs("Jane <jane@acme.com>, raw@acme.com")
...
def test_resolve_owner_email_in_header_first():
    ...
    email = agent.resolve_owner_email("Mike", "Mike Smith <mike@acme.com>, Other <other@acme.com>")
...
def test_list_contacts_search(client):
    client.post(
        "/contacts", json={"name": "Jane Doe", "employer": "Acme", "emails": ["jane@acme.com"]}
    )
    client.post(
        "/contacts", json={"name": "Bob Smith", "employer": "Globex", "emails": ["bob@globex.com"]}
    )
