...
def test_create_card_rejects_inverted_dates(client):
    _wipe_lookahead()
    r = client.post(
        "/lookahead/cards", json=_card_payload(start_date="2026-04-20", end_date="2026-04-18")
    )
    assert r.status_code == 400
...
def test_list_cards_filters_by_project_and_window(client):
    ...
    client.post(
        "/lookahead/cards",
        json=_card_payload(
            title="A", project="P905", start_date="2026-04-10", end_date="2026-04-11"
        ),
    )
    client.post(
        "/lookahead/cards",
        json=_card_payload(
            title="B", project="P905", start_date="2026-04-20", end_date="2026-04-21"
        ),
    )
    client.post(
        "/lookahead/cards",
        json=_card_payload(
            title="C", project="OTHER", start_date="2026-04-15", end_date="2026-04-15"
        ),
    )
    ...
    windowed = client.get("/lookahead/cards?project=P905&start=2026-04-15&end=2026-04-25").json()
...
def test_card_dependencies_are_round_trippable(client):
    ...
    b = client.post("/lookahead/cards", json=_card_payload(title="B", depends_on=[a["id"]])).json()
...
def test_card_links_validate_type(client):
    ...
    card = client.post(
        "/lookahead/cards",
        json=_card_payload(links=[{"type": "todo", "id": "42"}, {"type": "bogus", "id": "1"}]),
    ).json()
...
def test_card_bom_and_resource_status_update(client):
    _wipe_lookahead()
    bob = client.post("/lookahead/resources", json={"name": "Bob", "type": "person"}).json()
    card = client.post(
        "/lookahead/cards",
        json=_card_payload(
            resources=[{"resource_id": bob["id"], "quantity": 1, "status": "needed"}]
        ),
    ).json()
    ...
    r = client.patch(
        f"/lookahead/cards/{card['id']}/resources/{bob['id']}", json={"status": "secured"}
    )
    updated = r.json()
...
def test_resource_crud_cycle(client):
    _wipe_lookahead()
    r = client.post("/lookahead/resources", json={"name": "Crane", "type": "equipment"})
    res = r.json()
    assert res["name"] == "Crane" and res["type"] == "equipment"

    client.patch(f"/lookahead/resources/{res['id']}", json={"notes": "5-ton"})
    fetched = [x for x in client.get("/lookahead/resources").json() if x["id"] == res["id"]][0]
    assert fetched["notes"] == "5-ton"

    client.delete(f"/lookahead/resources/{res['id']}")
    remaining = [x for x in client.get("/lookahead/resources").json() if x["id"] == res["id"]]
    assert remaining == []


def test_resource_rejects_invalid_type(client):
    _wipe_lookahead()
    r = client.post("/lookahead/resources", json={"name": "X", "type": "nonsense"})
    assert r.status_code == 400
...
def test_overview_groups_and_sorts_by_earliest(client):
    ...
    client.post(
        "/lookahead/cards",
        json=_card_payload(project="LATE", start_date="2026-04-20", end_date="2026-04-21"),
    )
    client.post(
        "/lookahead/cards",
        json=_card_payload(project="EARLY", start_date="2026-04-10", end_date="2026-04-11"),
    )
...
def test_template_rejects_invalid_duration_unit(client):
    _wipe_lookahead()
    r = client.post("/lookahead/templates", json=_template_payload(duration_unit="weeks"))
    assert r.status_code == 400
...
def test_instantiate_materializes_cards_with_correct_dates(client):
    ...
    tpl = client.post("/lookahead/templates", json=_template_payload()).json()
    r = client.post(
        f"/lookahead/templates/{tpl['id']}/instantiate",
...
def test_instantiate_business_days_skips_weekends(client):
    _wipe_lookahead()
    body = _template_payload(
        duration_unit="business_days",
        tasks=[{"local_id": "x", "title": "X", "offset_start_days": 3, "duration_shifts": 1}],
    )
...
def test_card_work_days_round_trips(client):
    _wipe_lookahead()
    card = client.post("/lookahead/cards", json=_card_payload(work_days="M,T,W,Th,F")).json()
...
def test_instantiate_sunday_wednesday_workweek(client):
    ...
    inst = client.post(
        f"/lookahead/templates/{tpl['id']}/instantiate",
        json={"start_date": "2026-05-03", "project_tag": "P905"},
    ).json()
...
def test_reschedule_honors_per_card_work_days(client):
    ...
    body = _template_payload(
        duration_unit="calendar_days",
        tasks=[
            {"local_id": "A", "title": "Any day", "offset_start_days": 0, "duration_shifts": 1},
            {
                "local_id": "B",
                "title": "Weekdays only",
                "offset_start_days": 0,
    ...
    rescheduled = client.patch(
        f"/lookahead/instances/{inst['id']}", json={"start_date": "2026-05-10"}
    ).json()
...
def test_instantiate_copies_named_resources_to_bom(client):
    _wipe_lookahead()
    crane = client.post("/lookahead/resources", json={"name": "Crane", "type": "equipment"}).json()
    body = _template_payload(
        tasks=[
            {
                "local_id": "x",
                ...
                "resource_requirements": [
                    {"resource_type": "equipment", "named_resource_id": crane["id"], "quantity": 1},
...
def test_reschedule_instance_shifts_all_cards(client):
    ...
    r = client.patch(f"/lookahead/instances/{inst['id']}", json={"start_date": "2026-05-11"})
    rescheduled = r.json()
    by_local = {c["template_task_local_id"]: c for c in rescheduled["cards"]}
...
def test_detach_card_keeps_it_out_of_reschedule(client):
    ...
    detached_card = [c for c in inst["cards"] if c["template_task_local_id"] == "A"][0]
    client.post(f"/lookahead/cards/{detached_card['id']}/detach")
    client.patch(f"/lookahead/instances/{inst['id']}", json={"start_date": "2026-05-11"})
...
def test_instance_autocompletes_when_all_cards_done(client):
    ...
    # Flip the last one.
    client.patch(f"/lookahead/cards/{inst['cards'][-1]['id']}", json={"status": "done"})
...
def test_upgrade_instance_refreshes_template_fields_and_preserves_user_edits(client):
    ...
    client.patch(
        f"/lookahead/cards/{card_a['id']}",
        json={"assignee": "Bob", "notes": "Bob is on it", "status": "in_progress"},
    )
...
def test_upgrade_instance_adds_missing_required_resource_without_dropping_user_bom(client):
    ...
    crane = client.post("/lookahead/resources", json={"name": "Crane", "type": "equipment"}).json()
    welder = client.post(
        "/lookahead/resources", json={"name": "Welder", "type": "equipment"}
    ).json()
    body = _template_payload(
        tasks=[
            {
                    ...
                    "resource_requirements": [
                        {
                            ...
                            "named_resource_id": crane["id"],
                            "quantity": 1,
                        },
                        {
                            "resource_type": "equipment",
                            "named_resource_id": welder["id"],
                            "quantity": 1,
                        },
                    ],
                }
...
def test_annotate_card_persists_suggestions_from_llm(client):
    ...
    card = client.post(
        "/lookahead/cards", json=_card_payload(title="Panel install", project="P905")
    ).json()
...
def test_annotate_skips_already_linked_items(client):
    ...
    card = client.post(
        "/lookahead/cards",
        json=_card_payload(title="X", project="P905", links=[{"type": "item", "id": "em-1"}]),
    ).json()

    with patch("app._llm.generate", return_value='[{"item_id": "em-1", "reason": "x"}]'):
        r = client.post(f"/lookahead/cards/{card['id']}/annotate")
...
def test_annotate_is_idempotent_per_target(client):
    ...
    card = client.post("/lookahead/cards", json=_card_payload(title="X", project="P905")).json()
...
def test_accept_suggestion_creates_card_link(client):
    ...
    card = client.post("/lookahead/cards", json=_card_payload(title="X", project="P905")).json()
    with patch("app._llm.generate", return_value='[{"item_id": "em-1", "reason": "r"}]'):
        client.post(f"/lookahead/cards/{card['id']}/annotate")
...
def test_annotate_project_fans_out_across_cards(client):
    ...
    with patch("app._llm.generate", return_value='[{"item_id": "em-1", "reason": "r"}]'):
        r = client.post("/lookahead/annotate-project", json={"project": "P905"})
    body = r.json()
...
def test_creating_card_creates_linked_todo(client):
    _wipe_lookahead()
    _clear_todos()
    card = client.post("/lookahead/cards", json=_card_payload(title="Weld frame")).json()
...
def test_reopening_todo_reopens_linked_card(client):
    _wipe_lookahead()
    _clear_todos()
    card = client.post("/lookahead/cards", json=_card_payload(status="done")).json()
    todo_id = db.get_card_todo_id(card["id"])
...
def test_user_links_preserve_todo_link(client):
    ...
    client.patch(
        f"/lookahead/cards/{card['id']}", json={"links": [{"type": "situation", "id": "sit-1"}]}
    )
...
def test_backfill_creates_todos_for_orphan_cards():
    ...
    db.conn().execute("DELETE FROM model_state WHERE key = ?", ("lookahead_todo_backfill_v1",))
